...
def do_print(widget,lines):
    ...
    print int(round((len(lines)/46)+.5))
    print_op.set_n_pages(int(round((len(lines)/46)+.5)))
...
def draw_page(operation, context, page_nr, user_data=None):
        ...
        text_string="".join(user_data[page_nr*46:(page_nr+1)*46])
        ...
        #print layout.get_iter().get_line_xrange()
...
lines=["This is a very very very long line to test that wraping lines works, somewhat nicely\n",]
for num in range(1,179):
    lines.append("this is line %i\n" % num)
b.connect("clicked", do_print, lines)
window.add(b)
window.show_all()
gtk.main()
