...
class NewsItem:
    ...
    def __str__(self):
        return " ".join([self.title,',',self.author,',',self.url])

def get_current_selected(*args,**keywords):
    s=objectlist.get_selected()
    print s
...
vbox=gtk.VBox()
vbox.add(objectlist)
b=gtk.Button("clickme")
b.connect("clicked",get_current_selected)
vbox.add(b)
